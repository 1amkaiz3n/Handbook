...
Untuk identifikasi NS kita bisa menggunakan `host`, `dig`
...
Dari sini kita dapat 2 authoritative name server:

* `nsztm1.digi.ninja`
* `nsztm2.digi.ninja`

Name server ini akan kita test apakah mengizinkan **Zone Transfer (AXFR)**, yaitu mekanisme yang seharusnya hanya boleh untuk secondary DNS.
...
```bash
; <<>> DiG 9.18.39-0ubuntu0.24.04.5-Ubuntu <<>> axfr zonetransfer.me @nsztm1.digi.ninja.
;; global options: +cmd
...
;; WHEN: Sun May 31 06:53:57 WIB 2026
...
Zone transfer **BERHASIL** → artinya DNS server mengizinkan full zone dump.
...
* Subdomain internal:
  ...
  * `owa.zonetransfer.me`
* IP address mapping lengkap semua host
* Name server internal (`intns1`, `intns2`)
* Host lokal / testing (`127.0.0.1` entries juga ikut leak)
